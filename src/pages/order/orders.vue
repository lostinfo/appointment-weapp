<template>
  <view>
    ...
    <scroll-view scroll-x class="bg-white nav fixed" scroll-with-animation :scroll-left="scrollLeft"
      ...
      <view class="cu-item" :class="index == order_status_index ? 'text-green cur':''"
            v-for="(order_status, index) in order_status_list" :key="index" @tap="handleOrderStatusChange"
            :data-index="index">
        {{order_status.label}}
      </view>
    </scroll-view>
    <navigator class="margin-lr-sm margin-top-sm radius" v-for="(order, index) in orders" :key="index"
      ...
      <view class="bg-white">
        ...
        <view class="padding-sm flex">
          <image :src="getPath(order.product_thumb)" mode="aspectFill" class="radius"
  ...
  export default {
    ...
    methods: {
      ...
      getOrders() {
        ...
        that.$http.get('/orders', query).then(res => {
          that.loading = false
